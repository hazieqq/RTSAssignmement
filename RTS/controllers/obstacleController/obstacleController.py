...
for x in range(num_left_dist_sensors):
    dist_left_sensors.append(robot.getDistanceSensor('ps' + str(x)))
    

list(map((lambda s: s.enable(timeStep)), dist_left_sensors))  # Enable all distance sensors

dist_right_sensors  = []
for x in range(num_right_dist_sensors):
    dist_right_sensors.append(robot.getDistanceSensor('ps' + str(x)))
